...
class isys_report_view_cable_connections extends isys_report_view
{
        ...
        if ($resource->num_rows() > 0) {
            while ($row = $resource->get_row()) {
                ...
                if ($row['usages'] !== self::USED_CABLE) {
                    ...
                    $result[self::$usageToLanguageVariable[$row['usages']]][$row['isys_obj__id']]['quickInfoLink'] = $quickInfo->get_quick_info($row['isys_obj__id'],
                        $row['isys_obj__title'], C__LINK__CATG, false, [
    ...
    {
        return "view_cable_connections.tpl";
    }
